...
public class CaklerahEntity extends Monster implements GeoEntity {
    ...
    public CaklerahEntity(EntityType<CaklerahEntity> type, Level world) {
        super(type, world);
        xpReward = 0;
        setNoAi(false);
    ...
    protected void registerGoals() {
        ...
        this.goalSelector.addGoal(3, new RandomStrollGoal(this, 3, 20) {
            ...
            protected Vec3 getPosition() {
                RandomSource random = CaklerahEntity.this.getRandom();
                double dir_x = CaklerahEntity.this.getX() + ((random.nextFloat() * 2 - 1) * 16);
                double dir_y = CaklerahEntity.this.getY() + ((random.nextFloat() * 2 - 1) * 16);
                double dir_z = CaklerahEntity.this.getZ() + ((random.nextFloat() * 2 - 1) * 16);
        ...
        this.goalSelector.addGoal(4, new Goal() {
            ...
            public void tick() {
                LivingEntity livingentity = CaklerahEntity.this.getTarget();
                if (CaklerahEntity.this.getBoundingBox().intersects(livingentity.getBoundingBox())) {
                    CaklerahEntity.this.doHurtTarget(livingentity);
                } else {
                    double d0 = CaklerahEntity.this.distanceToSqr(livingentity);
                    if (d0 < 16) {
                        Vec3 vec3d = livingentity.getEyePosition(1);
                        CaklerahEntity.this.moveControl.setWantedPosition(vec3d.x, vec3d.y, vec3d.z, 1);
                    }
